...
from main import requests, QuadroGeralInabilitadosService
...
class TestScrapping(unittest.TestCase):
    ...
    def test_request_url_success(self):
        response_url = self.service_qgi.request_url(self.url)
    ...
    def test_mock_request_url_timeout(self, mock_request):
        with self.assertRaises(requests.exceptions.Timeout):
            self.service_qgi.request_url(self.url)
    ...
    def test_qgi_table_and_quantity(self):
        ...
        self.assertEqual(qgi_table_data.__len__(), 804)
